...
const AcademicCalendarScreen = () => {
  ...
  const [uiScheduleItem, setUiScheduleItem] = useState<ScheduleItemType[]>([]);
  ...
  const {data: allScheduleItemData, refetch: refetchAllSchedule} = useQuery<
    ISchedule[]
  >({
  ...
  useEffect(() => {
    if (!uiScheduleItem) return;
    const tmpCheckedList = uiScheduleItem.map(item => {
      return false;
    });
  ...
  // 북마크 기능이 활설화 됨
  useEffect(() => {
    if (!allScheduleItemData) return;
    const ui: ScheduleItemType[] = allScheduleItemData.map(scheduleItem => {
      return {...scheduleItem, onClick: bookmarkHandler};
    });
    setUiScheduleItem(ui);
  }, [allScheduleItemData]);
  ...
  return (
    <View>
      ...
      <S.ScheduleContainer>
        {uiScheduleItem &&
          uiScheduleItem.map((scheduleItem, idx: number) => {
            return (
              <ScheduleItem
